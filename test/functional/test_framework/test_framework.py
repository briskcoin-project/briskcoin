...
TMPDIR_PREFIX = "briskcoin_func_test_"
...
class BriskcoinTestMetaClass(type):
    """Metaclass for BriskcoinTestFramework.

    Ensures that any attempt to register a subclass of `BriskcoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'BriskcoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("BriskcoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("BriskcoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class BriskcoinTestFramework(metaclass=BriskcoinTestMetaClass):
    """Base class for a briskcoin test script.

    Individual briskcoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave briskcoinds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop briskcoinds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use briskcoin-cli instead of RPC for all commands")
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: briskcoinds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("BriskcoinRPC")
    ...
    def stop_nodes(self, wait=0):
        """Stop multiple briskcoind test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the briskcoind
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in briskcoin.conf
    ...
    def skip_if_no_briskcoind_tracepoints(self):
        """Skip the running test if briskcoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("briskcoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_briskcoind_zmq(self):
        """Skip the running test if briskcoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("briskcoind has not been built with zmq enabled.")
