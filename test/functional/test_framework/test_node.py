#!/usr/bin/env python3
# Copyright (c) 2017-present The Briskcoin Core developers
...
# 'briskcoin' command, it may internally insert more args, which must be
    ...
    def wait_for_rpc_connection(self, *, wait_for_import=True):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            ...
            try:
                ...
                # Suppress if cookie file isn't generated yet and no rpcuser or rpcpassword; briskcoind may be starting.
                if "No RPC credentials" not in str(e):
                    raise
                latest_error = suppress_error("missing_credentials", e)
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error(f"Unable to connect to briskcoind after {self.rpc_timeout}s (ignored errors: {dict(suppressed_errors)!s}{'' if latest_error is None else f', latest: {latest_error[0]!r}/{latest_error[1]}'})")
