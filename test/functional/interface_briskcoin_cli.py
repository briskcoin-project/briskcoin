...
# The block reward of coinbaseoutput.nValue (50) BKC/block matures after
# COINBASE_MATURITY (100) blocks. Therefore, after mining 101 blocks we expect
# node 0 to have a balance of (BLOCKS - COINBASE_MATURITY) * 50 BKC/block.
...
WALLET_NOT_SPECIFIED = (
    ...
    "option before the command (run \"briskcoin-cli -h\" for help or \"briskcoin-cli listwallets\" to see "
...
class TestBriskcoinCli(BriskcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from getblockchaininfo RPC and `briskcoin-cli getblockchaininfo`")
        ...
        # Prevent briskcoin-cli from using existing rpcport in conf
        ...
        self.log.info("Test -getinfo and briskcoin-cli return all proxies")
        ...
        if self.is_wallet_compiled():
            self.log.info("Test -getinfo and briskcoin-cli getwalletinfo return expected wallet info")
            ...
            assert_equal(Decimal(cli_get_info['Transaction fee rate (-paytxfee) (BKC/kvB)']), wallet_info['paytxfee'])
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (BKC/kvB)']), network_info['relayfee'])
            ...
            # Mine a block to confirm; adds a block reward (50 BKC) to the default wallet.
            ...
            # Test briskcoin-cli -generate with -rpcwallet in multiwallet mode.
...
if __name__ == '__main__':
    TestBriskcoinCli(__file__).main()
