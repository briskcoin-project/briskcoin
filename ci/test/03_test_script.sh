...
BRISKCOIN_CONFIG_ALL="-DBUILD_BENCH=ON -DBUILD_FUZZ_BINARY=ON"
if [ -z "$NO_DEPENDS" ]; then
  BRISKCOIN_CONFIG_ALL="${BRISKCOIN_CONFIG_ALL} -DCMAKE_TOOLCHAIN_FILE=$DEPENDS_DIR/$HOST/toolchain.cmake"
fi
if [ -z "$NO_WERROR" ]; then
  BRISKCOIN_CONFIG_ALL="${BRISKCOIN_CONFIG_ALL} -DWERROR=ON"
fi
...
BRISKCOIN_CONFIG_ALL="$BRISKCOIN_CONFIG_ALL -DENABLE_EXTERNAL_SIGNER=ON -DCMAKE_INSTALL_PREFIX=$BASE_OUTDIR"

if [[ "${RUN_TIDY}" == "true" ]]; then
  BRISKCOIN_CONFIG_ALL="$BRISKCOIN_CONFIG_ALL -DCMAKE_EXPORT_COMPILE_COMMANDS=ON"
fi

bash -c "cmake -S $BASE_ROOT_DIR $BRISKCOIN_CONFIG_ALL $BRISKCOIN_CONFIG || ( (cat $(cmake -P "${BASE_ROOT_DIR}/ci/test/GetCMakeLogFiles.cmake")) && false)"
...
if [ "${RUN_TIDY}" = "true" ]; then
  ...
  python3 "/include-what-you-use/iwyu_tool.py" \
           ...
           -- -Xiwyu --cxx17ns -Xiwyu --mapping_file="${BASE_ROOT_DIR}/contrib/devtools/iwyu/briskcoin.core.imp" \
