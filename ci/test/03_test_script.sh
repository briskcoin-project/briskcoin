...
BRISKCOIN_CONFIG_ALL="$BRISKCOIN_CONFIG_ALL -DCMAKE_INSTALL_PREFIX=$BASE_OUTDIR -Werror=dev"

if [[ "${RUN_TIDY}" == "true" ]]; then
  BRISKCOIN_CONFIG_ALL="$BRISKCOIN_CONFIG_ALL -DCMAKE_EXPORT_COMPILE_COMMANDS=ON"
fi

bash -c "cmake -S $BASE_ROOT_DIR -B ${BASE_BUILD_DIR} $BRISKCOIN_CONFIG_ALL $BRISKCOIN_CONFIG" || (
  cd "${BASE_BUILD_DIR}"
