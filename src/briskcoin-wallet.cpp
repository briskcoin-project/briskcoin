...
static std::optional<int> WalletAppInit(ArgsManager& args, int argc, char* argv[])
{
    ...
    if (missing_args || HelpRequested(args) || args.IsArgSet("-version")) {
        std::string strUsage = strprintf("%s briskcoin-wallet version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n";

        if (args.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                        "briskcoin-wallet is an offline tool for creating and interacting with " PACKAGE_NAME " wallet files.\n"
                        ...
                        "Usage:\n"
                        "  briskcoin-wallet [options] <command>\n";
            strUsage += "\n" + args.GetHelpMessage();
        }
