// Copyright (c) 2025 The Briskcoin Core developers
...
Options:
  -m, --multiprocess     Run multiprocess binaries briskcoin-node, briskcoin-gui.
  -M, --monolithic       Run monolithic binaries briskcoind, briskcoin-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'briskcoin-qt [ARGS]' or 'briskcoin-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'briskcoind [ARGS]' or 'briskcoin-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'briskcoin-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'briskcoin-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'briskcoin-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_briskcoin [ARGS]'.
...
int main(int argc, char* argv[])
{
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(cmd.use_multiprocess ? "briskcoin-gui" : "briskcoin-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(cmd.use_multiprocess ? "briskcoin-node" : "briskcoind");
        } else if (cmd.command == "rpc") {
            args.emplace_back("briskcoin-cli");
            // Since "briskcoin rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("briskcoin-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("briskcoin-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_briskcoin");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("briskcoin-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_briskcoin");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_briskcoin-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("briskcoin-util");
        } else {
...
//! Execute the specified briskcoind, briskcoin-qt or other command line in `args`
...
//!             be a relative path to a briskcoind, briskcoin-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/briskcoin ...
...
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    // (https://github.com/briskcoin/briskcoin/pull/31375#discussion_r1861814807)
