...
const char * const BITCOIN_CONF_FILENAME = "briskcoin.conf";
const char * const BITCOIN_PID_FILENAME = "briskcoind.pid";
const char * const DEFAULT_DEBUGLOGFILE = "debug.log";

ArgsManager gArgs;
bool fPrintToConsole = false;
bool fPrintToDebugLog = true;
bool fPruneDebugLog = false; // FIXME.BKC // prune debug.log
...
#else
    const char* pszModule = "briskcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Briskcoin";
#else
    fs::path pathRet;
