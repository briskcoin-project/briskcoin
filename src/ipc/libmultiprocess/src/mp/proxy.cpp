...
Connection::~Connection()
{
    ...
    // object, which can run arbitrary blocking briskcoin code so they have to run
    ...
    // functions run, so client objects are fully disconnected before briskcoin
    // code in the destructors are run. This way if the briskcoin code tries to
...
void EventLoop::addAsyncCleanup(std::function<void()> fn)
{
    ...
    //
    // In briskcoin core, running these callbacks in the right order is
