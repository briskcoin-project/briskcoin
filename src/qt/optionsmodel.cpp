...
/** Call node.updateRwSetting() with Briskcoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Briskcoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/briskcoin/briskcoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because briskcoin 22.x will not try to read these.
...
//! Convert enabled/size values to briskcoin -prune setting.
...
//! Get pruning enabled value to show in GUI from briskcoin -prune setting.
...
//! Get pruning size value to show in GUI from briskcoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayBriskcoinUnit")) {
        settings.setValue("DisplayBriskcoinUnit", QVariant::fromValue(BriskcoinUnit::BKC));
    }
    QVariant unit = settings.value("DisplayBriskcoinUnit");
    if (unit.canConvert<BriskcoinUnit>()) {
        m_display_briskcoin_unit = unit.value<BriskcoinUnit>();
    } else {
        m_display_briskcoin_unit = BriskcoinUnit::BKC;
        settings.setValue("DisplayBriskcoinUnit", QVariant::fromValue(m_display_briskcoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if briskcoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_briskcoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<BriskcoinUnit>() == m_display_briskcoin_unit) return;
    m_display_briskcoin_unit = new_unit.value<BriskcoinUnit>();
    QSettings settings;
    settings.setValue("DisplayBriskcoinUnit", QVariant::fromValue(m_display_briskcoin_unit));
    Q_EMIT displayUnitChanged(m_display_briskcoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/briskcoin/briskcoin/pull/8273
    ...
    // (https://github.com/briskcoin-core/gui/issues/567).
