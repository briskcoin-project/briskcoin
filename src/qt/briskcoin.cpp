...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<BriskcoinUnit>("BriskcoinUnit");
#else
    qRegisterMetaType<BriskcoinUnit>("BriskcoinUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. briskcoin_de.qm (shortcut "de" needs to be defined in briskcoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. briskcoin_de_DE.qm (shortcut "de_DE" needs to be defined in briskcoin.qrc)
...
static const char* qt_argv = "briskcoin-qt";

BriskcoinApplication::BriskcoinApplication()
...
void BriskcoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the BriskcoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", BriskcoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void BriskcoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool BriskcoinApplication::createOptionsModel(bool resetSettings)
{
...
void BriskcoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new BriskcoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &BriskcoinGUI::quitRequested, this, &BriskcoinApplication::requestShutdown);
...
void BriskcoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void BriskcoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool BriskcoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void BriskcoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &BriskcoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &BriskcoinApplication::handleRunawayException);
    connect(this, &BriskcoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &BriskcoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void BriskcoinApplication::parameterSetup()
{
...
void BriskcoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void BriskcoinApplication::requestInitialize()
{
...
void BriskcoinApplication::requestShutdown()
{
...
void BriskcoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // briskcoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &BriskcoinGUI::handlePaymentRequest);
            connect(window, &BriskcoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void BriskcoinApplication::handleRunawayException(const QString &message)
{
...
void BriskcoinApplication::handleNonFatalException(const QString& message)
{
...
WId BriskcoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool BriskcoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", BriskcoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(briskcoin);
    Q_INIT_RESOURCE(briskcoin_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(BRISKCOIN_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see briskcoin-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see briskcoin-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse briskcoin.conf, determine network, switch to network specific
    ...
    // briskcoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
