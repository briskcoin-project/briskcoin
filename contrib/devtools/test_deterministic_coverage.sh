...
TEST_BRISKCOIN_BINARY="src/test/test_briskcoin"
...
if [[ ! -e ${TEST_BRISKCOIN_BINARY} ]]; then
    echo "Error: Executable ${TEST_BRISKCOIN_BINARY} not found. Run \"cmake -B build -DCMAKE_BUILD_TYPE=Coverage\" and compile."
    exit 1
fi
...
while [[ ${TEST_RUN_ID} -lt ${N_TEST_RUNS} ]]; do
    ...
    if ! BOOST_TEST_RUN_FILTERS="${BOOST_TEST_RUN_FILTERS}" ${TEST_BRISKCOIN_BINARY} > "${TEST_OUTPUT_TEMPFILE}" 2>&1; then
