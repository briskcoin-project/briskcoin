...
BRISKCOIND=${BRISKCOIND:-$BINDIR/briskcoind}
SHARE_EXAMPLES_DIR=${SHARE_EXAMPLES_DIR:-$TOPDIR/share/examples}
EXAMPLE_CONF_FILE=${EXAMPLE_CONF_FILE:-$SHARE_EXAMPLES_DIR/briskcoin.conf}

[ ! -x "$BRISKCOIND" ] && echo "$BRISKCOIND not found or not executable." && exit 1

DIRTY=""
VERSION_OUTPUT=$($BRISKCOIND --version)
if [[ $VERSION_OUTPUT == *"dirty"* ]]; then
  DIRTY="${DIRTY}${BRISKCOIND}\n"
fi

if [ -n "$DIRTY" ]
then
  echo -e "WARNING: $BRISKCOIND was built from a dirty tree.\n"
  echo -e "To safely generate a briskcoin.conf file, please commit your changes to $BRISKCOIND, rebuild, then run this script again.\n"
fi

echo 'Generating example briskcoin.conf file in share/examples/'
...
##
## briskcoin.conf configuration file.
## Generated by contrib/devtools/gen-briskcoin-conf.sh.
##
...
# parse the output from briskcoind --help
