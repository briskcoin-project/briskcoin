#!/usr/bin/env bash
#
# Copyright (c) 2019-2023 The Briskcoin Core developers
...
if (( $# < 3 )); then
  echo 'Usage: utxo_snapshot.sh <generate-at-height> <snapshot-out-path> <briskcoin-cli-call ...>'
  echo
  ...
  echo 'Examples:'
  echo
  echo "  ./contrib/devtools/utxo_snapshot.sh 570000 utxo.dat ./src/briskcoin-cli -datadir=\$(pwd)/testdata"
  echo '  ./contrib/devtools/utxo_snapshot.sh 570000 - ./src/briskcoin-cli'
  exit 1
fi
...
BRISKCOIN_CLI_CALL="${*} -rpcclienttimeout=9999999"

# Check if the node is pruned and get the pruned block height
PRUNED=$( ${BRISKCOIN_CLI_CALL} getblockchaininfo | awk '/pruneheight/ {print $2}' | tr -d ',' )
...
CURRENT_BLOCK_HEIGHT=$(${BRISKCOIN_CLI_CALL} getblockcount)
...
function cleanup {
  ...
  ${BRISKCOIN_CLI_CALL} reconsiderblock "${PIVOT_BLOCKHASH}"

  if $NETWORK_DISABLED; then
    (>&2 echo "Restoring network activity")
    ${BRISKCOIN_CLI_CALL} setnetworkactive true
  fi
}
...
if [[ "$REPLY" =~ ^[Yy]*$ || -z "$REPLY" ]]; then
  ...
  ${BRISKCOIN_CLI_CALL} setnetworkactive false
else
  (>&2 echo "Network activity remains enabled")
fi

# Block we'll invalidate/reconsider to rewind/fast-forward the chain.
PIVOT_BLOCKHASH=$($BRISKCOIN_CLI_CALL getblockhash $(( GENERATE_AT_HEIGHT + 1 )) )
...
${BRISKCOIN_CLI_CALL} invalidateblock "${PIVOT_BLOCKHASH}"

if [[ "${OUTPUT_PATH}" = "-" ]]; then
  (>&2 echo "Generating txoutset info...")
  ${BRISKCOIN_CLI_CALL} gettxoutsetinfo | grep hash_serialized_3 | sed 's/^.*: "\(.\+\)\+",/\1/g'
else
  (>&2 echo "Generating UTXO snapshot...")
  ${BRISKCOIN_CLI_CALL} dumptxoutset "${OUTPUT_PATH}"
fi
