...
Tracing (USDT) support for Briskcoin Core on Linux using. For more information on
USDT support in Briskcoin Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `briskcoind` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `briskcoind` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Briskcoin Core USDT documentation] on how to list available tracepoints in your
`briskcoind` binary.

[Briskcoin Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py $(pidof briskcoind)
```
...
Briskcoin P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py $(pidof briskcoind)
```
...
In a different terminal, starting Briskcoin Core in SigNet mode and with
re-indexing enabled.

```
$ ./build/bin/briskcoind -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py $(pidof briskcoind)
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py $(pidof briskcoind)
```
