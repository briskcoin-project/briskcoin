...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing briskcoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='briskcoin-cli', help='briskcoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Briskcoin address to which the faucet should send')
...
parser.add_argument('briskcoin_cli_args', nargs='*', help='Arguments to pass on to briskcoin-cli (default: -signet)')

args = parser.parse_args()

if args.briskcoin_cli_args == []:
    args.briskcoin_cli_args = ['-signet']


def briskcoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.briskcoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = briskcoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = briskcoin_cli(['getnewaddress', 'faucet', 'bech32'])
