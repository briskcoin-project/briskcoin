...
"""Script for verifying Briskcoin Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from briskcoincore.org and briskcoin.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/briskcoin-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://briskcoincore.org"
HOST2 = "https://briskcoin.org"
VERSIONPREFIX = "briskcoin-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Briskcoin community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"briskcoin_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by briskcoincore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the briskcoin release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes briskcoin.org lags behind briskcoincore.org.)')
    )
