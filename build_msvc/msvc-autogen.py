...
def parse_config_into_bkc_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Briskcoin Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/briskcoin_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/briskcoin_config.h'), "w", encoding="utf8") as bkc_config:
        bkc_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='Briskcoin-core msbuild configuration initialiser.')
    ...
    parse_config_into_bkc_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/briskcoin_config.h'), os.path.join(SOURCE_DIR, 'config/briskcoin-config.h'))
