...
```sh
[offline]$ ./src/briskcoin-cli -signet -named createwallet \
...
```sh
[offline]$ ./src/briskcoin-cli -signet -rpcwallet="offline_wallet" listdescriptors \
             | jq -r '.descriptors' \
...
```sh
[online]$ ./src/briskcoin-cli -signet -named createwallet \
...
```sh
[online]$ ./src/briskcoin-cli -signet -rpcwallet="watch_only_wallet" importdescriptors "$(cat /path/to/descriptors.json)"
...
```sh
[online]$ ./src/briskcoin-cli -signet -rpcwallet="watch_only_wallet" getnewaddress
...
```sh
[online]$ ./src/briskcoin-cli -signet -rpcwallet="watch_only_wallet" listunspent
...
```sh
[online]$ ./src/briskcoin-cli -signet -rpcwallet="watch_only_wallet" send \
...
```sh
[offline]$ ./src/briskcoin-cli -signet decodepsbt $(cat /path/to/funded_psbt.txt)

{
    ...
}

[offline]$ ./src/briskcoin-cli -signet analyzepsbt $(cat /path/to/funded_psbt.txt)
...
```sh
[offline]$ ./src/briskcoin-cli -signet -rpcwallet="offline_wallet" walletpassphrase "** enter passphrase **" 60
```
...
2. Process, sign and finalize the PSBT on the `offline_wallet` using the `walletprocesspsbt` command, saving the output to a file `final_psbt.txt`.
 ...
 ```sh
[offline]$ ./src/briskcoin-cli -signet -rpcwallet="offline_wallet" walletprocesspsbt \
...
```sh
[online]$ ./src/briskcoin-cli -signet sendrawtransaction $(cat /path/to/final_psbt.txt)
...
```sh
[online]$ ./src/briskcoin-cli -signet -rpcwallet="watch_only_wallet" getbalances
...
```sh
[online]$ ./src/briskcoin-cli -signet -rpcwallet="watch_only_wallet" listtransactions
