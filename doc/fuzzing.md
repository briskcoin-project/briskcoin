...
```sh
$ git clone --depth=1 https://github.com/briskcoin-core/qa-assets
...
an instrumentalized libc++, then using it to build [Briskcoin Core dependencies
from source](../depends/README.md) and finally the Briskcoin Core fuzz binary
...
the qa-assets repo
(https://github.com/briskcoin-core/qa-assets/blob/main/.github/PULL_REQUEST_TEMPLATE.md).
...
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://issues.oss-fuzz.com/issues?q=briskcoin-core%20status:open).

Briskcoin Core follows its [security disclosure policy](https://briskcoincore.org/en/security-advisories/),
