# PSBT Howto for Briskcoin Core

Since Briskcoin Core 0.17, an RPC interface exists for Partially Signed Briskcoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/briskcoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Briskcoin transactions that are not fully signed
...
#### Multisig with multiple Briskcoin Core instances
...
Briskcoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `briskcoin-cli` is used.
...
Later, when *V* BKC has been received on *Amulti*, and Bob and Carol want to
