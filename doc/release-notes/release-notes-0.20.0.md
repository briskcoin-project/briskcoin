...
Briskcoin Core version 0.20.0 is now available from:

  <https://briskcoincore.org/bin/briskcoin-core-0.20.0/>
...
- Instead of running `make` simply, you should instead run
  `BRISKCOIN_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from Briskcoin Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Briskcoin Core version 0.18.0.
...
- Testing or debugging of implementations of the Briskcoin P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Briskcoin Core. Briskcoin Core logs debug messages
...
- OpenSSL is no longer used by Briskcoin Core.  (#17265)

- BIP70 support has been fully removed from Briskcoin Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/briskcoin/briskcoin/issues/16599),
  [PR #16702](https://github.com/briskcoin/briskcoin/pull/16702), and the
  `briskcoind help` for more information.  This option is experimental and
...
- All custom settings configured when Briskcoin Core starts are now
...
- The "Start Briskcoin Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://briskcoincore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Briskcoin Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Briskcoin Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/briskcoin-core/HWI) for
  signing.  Future versions of Briskcoin Core should support a GUI option
...
- Briskcoin Core's automatically-generated source code documentation is
  now available at https://doxygen.briskcoincore.org.  (#17596)
...
- The `briskcoin-cli` utility used with the `-getinfo` parameter now
...
- The `briskcoin-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `briskcoinstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_briskcoin-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBBRISKCOIN_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_briskcoin` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBBRISKCOIN_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for briskcoin-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate briskcoin schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest briskcoin.conf (sdaftuar)
...
- #18022 Fix appveyor `test_briskcoin` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (briskcoin-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone briskcoin-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Briskcoin Core files (hebasto)
...
- #17119 Fix broken briskcoin-cli examples (andrewtoth)
...
- #17503 Remove bitness from briskcoin-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.briskcoincore.org (laanwj)
...
- #17648 Rename wallet-tool references to briskcoin-wallet (hel-o)
...
- #18341 Replace remaining literal BKC with `CURRENCY_UNIT` (domob1812)
...
- #16115 On briskcoind startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from briskcoin-cli -getinfo (malevolent)
