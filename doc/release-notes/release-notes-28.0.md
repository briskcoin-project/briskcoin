Briskcoin Core version 28.0 is now available from:

  <https://briskcoincore.org/bin/briskcoin-core-28.0/>
...
Running Briskcoin Core binaries on macOS requires self signing.
```
cd /path/to/briskcoin-28.0/bin
xattr -d com.apple.quarantine briskcoin-cli briskcoin-qt briskcoin-tx briskcoin-util briskcoin-wallet briskcoind test_briskcoin
codesign -s - briskcoin-cli briskcoin-qt briskcoin-tx briskcoin-util briskcoin-wallet briskcoind test_briskcoin
```
...
Support for Testnet4 as specified in [BIP94](https://github.com/briskcoin/bips/blob/master/bip-0094.mediawiki)
...
The default data directory on Windows has been moved from `C:\Users\Username\AppData\Roaming\Briskcoin`
to `C:\Users\Username\AppData\Local\Briskcoin`. Briskcoin Core will check the existence
...
See [JSON-RPC-interface.md](https://github.com/briskcoin/briskcoin/blob/master/doc/JSON-RPC-interface.md#json-rpc-11-vs-20) for details. (#27101)
...
libbriskcoinconsensus Removal
---------------------------

The libbriskcoin-consensus library was deprecated in 27.0 and is now completely removed. (#29648)

P2P and Network Changes
-----------------------

- Previously if Briskcoin Core was listening for P2P connections, either using
...
- Briskcoin Core will now fail to start up if any of its P2P binds fail, rather
...
- Transactions with version number set to 3 are now treated as standard on all networks (#29496),
  ...
  described in [BIP 431](https://github.com/briskcoin/bips/blob/master/bip-0431.mediawiki).  The
...
- The `warnings` field in `getblockchaininfo`, `getmininginfo` and
  ...
  can be temporarily restored by running Briskcoin Core with the configuration
...
are now required to compile Briskcoin Core. (#29091, #30263)

- The minimum required glibc to run Briskcoin Core is now
...
- A new `-testdatadir` option has been added to `test_briskcoin` to allow specifying the
...
Previous releases of Briskcoin Core or previous external software will not be able to read the blocksdir with a non-zero XOR-key.
