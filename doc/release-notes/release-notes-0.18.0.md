Briskcoin Core version 0.18.0 is now available from:

  <https://briskcoincore.org/bin/briskcoin-core-0.18.0/>
...
`/Applications/Briskcoin-Qt` (on Mac) or `briskcoind`/`briskcoin-qt` (on
Linux).
...
recommended to use Briskcoin Core on unsupported systems.

Briskcoin Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Briskcoin Core does not yet change appearance when
...
`contrib/init/briskcoind.service`, it has been changed to use
`/var/lib/briskcoind` as the data directory instead of
`~briskcoin/.briskcoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/briskcoind` will
exist has enough space (check using `df -h /var/lib/briskcoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/briskcoin/briskcoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://briskcoincore.org/en/contact/) the Briskcoin Core
...
The systemd init file (`contrib/init/briskcoind.service`) has been changed
to use `/var/lib/briskcoind` as the data directory instead of
`~briskcoin/.briskcoin`. This change makes Briskcoin Core more consistent with
...
`datadir` option in `/etc/briskcoin/briskcoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/briskcoin/briskcoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/briskcoin/briskcoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/briskcoin/briskcoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/briskcoin/briskcoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/briskcoin/briskcoin/blob/master/doc/briskcoin-conf.md)
  about the `briskcoin.conf` file describes how to use it to configure
  Briskcoin Core.

- A new document introduces Briskcoin Core's BIP174 [Partially-Signed
  Briskcoin Transactions
  (PSBT)](https://github.com/briskcoin/briskcoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/briskcoin/briskcoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Briskcoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Briskcoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/briskcoin/briskcoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/briskcoin/briskcoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Briskcoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Briskcoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/briskcoin/briskcoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/briskcoin/briskcoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart briskcoind with the `-deprecatedrpc=generate`
...
- A new `briskcoin-wallet` tool is now distributed alongside Briskcoin
...
This section describes planned changes to Briskcoin Core that may affect
other Briskcoin software and services.

- Since version 0.16.0, Briskcoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Briskcoin Core 0.20 (expected about
  a year after 0.18), Briskcoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Briskcoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Briskcoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Briskcoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Briskcoin Core
...
- A sub-project of Briskcoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Briskcoin Core.  See their [project
  page](https://github.com/briskcoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Briskcoin Core's own implementation, although entropy
  gathered by Briskcoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Briskcoin Core a
...
- On macOS, Briskcoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `briskcoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libbriskcoin_server.a` (jnewbery)
...
- #13926 [Tools] briskcoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like briskcoind (MarcoFalke)
...
- #14521 qt, docs: Fix `briskcoin-qt -version` output formatting (hebasto)
...
- #11625 Add BriskcoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libbriskcoin_server` (laanwj)
...
- #15154 Configure: briskcoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/briskcoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Briskcoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libbriskcoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in briskcoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Briskcoin Core (Christewart)
...
- #12246 Bugfix: Only run briskcoin-tx tests when briskcoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_briskcoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_briskcoin` (MarcoFalke)
...
- #14186 briskcoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_briskcoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update briskcoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/briskcoin-conf.md (hebasto)
...
- #15354 Add missing `briskcoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct briskcoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support briskcoin folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/briskcoin/).
