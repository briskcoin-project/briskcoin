...
for ((n=1;n<=3;n++))
do
 ./src/briskcoin-cli -signet createwallet "participant_${n}"
done
```
...
for ((n=1;n<=3;n++))
do
 xpubs["internal_xpub_${n}"]=$(./src/briskcoin-cli -signet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/1/*"))][0] | .desc' | grep -Po '(?<=\().*(?=\))')

 xpubs["external_xpub_${n}"]=$(./src/briskcoin-cli -signet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/0/*") )][0] | .desc' | grep -Po '(?<=\().*(?=\))')
done
```
...
external_desc_sum=$(./src/briskcoin-cli -signet getdescriptorinfo $external_desc | jq '.descriptor')
internal_desc_sum=$(./src/briskcoin-cli -signet getdescriptorinfo $internal_desc | jq '.descriptor')
...
Note that at least two descriptors are usually used, one for internal derivation paths and external ones. There are discussions about eliminating this redundancy, as can been seen in the issue [#17190](https://github.com/briskcoin/briskcoin/issues/17190).
...
Documentation for these and other parameters can be found by typing `./src/briskcoin-cli help importdescriptors`.
...
```bash
./src/briskcoin-cli -signet -named createwallet wallet_name="multisig_wallet_01" disable_private_keys=true blank=true

./src/briskcoin-cli  -signet -rpcwallet="multisig_wallet_01" importdescriptors "$multisig_desc"

./src/briskcoin-cli  -signet -rpcwallet="multisig_wallet_01" getwalletinfo
```
...
```bash
for ((n=1;n<=3;n++)); do ./src/briskcoin-cli -signet loadwallet "participant_${n}"; done
```
...
```bash
receiving_address=$(./src/briskcoin-cli -signet -rpcwallet="multisig_wallet_01" getnewaddress)

./contrib/signet/getcoins.py -c ./src/briskcoin-cli -a $receiving_address
```
...
```bash
./src/briskcoin-cli -signet -rpcwallet="multisig_wallet_01" getbalances
```
...
```bash
balance=$(./src/briskcoin-cli -signet -rpcwallet="multisig_wallet_01" getbalance)

amount=$(echo "$balance * 0.8" | bc -l | sed -e 's/^\./0./' -e 's/^-\./-0./')

destination_addr=$(./src/briskcoin-cli -signet -rpcwallet="participant_1" getnewaddress)

funded_psbt=$(./src/briskcoin-cli -signet -named -rpcwallet="multisig_wallet_01" walletcreatefundedpsbt outputs="{\"$destination_addr\": $amount}" | jq -r '.psbt')
```
...
```bash
./src/briskcoin-cli -signet decodepsbt $funded_psbt

./src/briskcoin-cli -signet analyzepsbt $funded_psbt
```
...
```bash
psbt_1=$(./src/briskcoin-cli -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq '.psbt')

psbt_2=$(./src/briskcoin-cli -signet -rpcwallet="participant_2" walletprocesspsbt $funded_psbt | jq '.psbt')
```
...
```bash
combined_psbt=$(./src/briskcoin-cli -signet combinepsbt "[$psbt_1, $psbt_2]")
```
...
```bash
finalized_psbt_hex=$(./src/briskcoin-cli -signet finalizepsbt $combined_psbt | jq -r '.hex')

./src/briskcoin-cli -signet sendrawtransaction $finalized_psbt_hex
```
...
```bash
psbt_1=$(./src/briskcoin-cli -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq -r '.psbt')

psbt_2=$(./src/briskcoin-cli -signet -rpcwallet="participant_2" walletprocesspsbt $psbt_1 | jq -r '.psbt')

finalized_psbt_hex=$(./src/briskcoin-cli -signet finalizepsbt $psbt_2 | jq -r '.hex')
