# CJDNS support in Briskcoin Core

It is possible to run Briskcoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of Briskcoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Briskcoin network and individual nodes.
...
before it will work with your Briskcoin Core node. This is described in steps
...
## Run Briskcoin Core with CJDNS

Once you are connected to the CJDNS network, the following Briskcoin Core
...
When enabled, this option tells Briskcoin Core that it is running in an
...
IPv6 local network. This helps Briskcoin Core perform better address management:
...
CJDNS support was added to Briskcoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `briskcoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/briskcoin/briskcoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Briskcoin Core

There are several ways to see your CJDNS address in Briskcoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `briskcoin-cli -netinfo 4`
