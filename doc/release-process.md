...
* Update manpages (after rebuilding the binaries), see [gen-manpages.py](https://github.com/briskcoin/briskcoin/blob/master/contrib/devtools/README.md#gen-manpagespy).
* Update briskcoin.conf and commit changes if they exist, see [gen-briskcoin-conf.sh](https://github.com/briskcoin/briskcoin/blob/master/contrib/devtools/README.md#gen-briskcoin-confsh).
...
* On the new release branch in [`configure.ac`](../configure.ac)(see [this commit](https://github.com/briskcoin/briskcoin/commit/742f7dd)):
...
* Update translations see [translation_process.md](https://github.com/briskcoin/briskcoin/blob/master/doc/translation_process.md#synchronising-translations).
* Update hardcoded [seeds](/contrib/seeds/README.md), see [this pull request](https://github.com/briskcoin/briskcoin/pull/27488) for an example.
* Update the following variables in [`src/kernel/chainparams.cpp`](/src/kernel/chainparams.cpp) for mainnet, testnet, and signet:
  ...
  - `chainTxData` with statistics about the transaction count and rate. Use the output of the `getchaintxstats` RPC with an
    ...
    [this pull request](https://github.com/briskcoin/briskcoin/pull/28591) for an example. Reviewers can verify the results by running
...
- Update the versions.
- Create the draft, named "*version* Release Notes Draft", as a [collaborative wiki](https://github.com/briskcoin-core/briskcoin-devwiki/wiki/_new).
- Clear the release notes: `cp doc/release-notes-empty-template.md doc/release-notes.md`
- Create a pinned meta-issue for testing the release candidate (see [this issue](https://github.com/briskcoin/briskcoin/issues/27621) for an example) and provide a link to it in the release announcements where useful.
- Translations on Transifex
    - Change the auto-update URL for the new major version's resource away from `master` and to the branch, e.g. `https://raw.githubusercontent.com/briskcoin/briskcoin/<branch>/src/qt/locale/briskcoin_en.xlf`. Do not forget this or it will keep tracking the translations on master instead, drifting away from the specific major release.
- Prune inputs from the qa-assets repo (See [pruning
  inputs](https://github.com/briskcoin-core/qa-assets#pruning-inputs)).

#### Before final release

- Merge the release notes from [the wiki](https://github.com/briskcoin-core/briskcoin-devwiki/wiki/) into the branch.
- Ensure the "Needs release note" label is removed from all relevant pull
  requests and issues:
  https://github.com/briskcoin/briskcoin/issues?q=label%3A%22Needs+release+note%22

#### Tagging a release (candidate)

To tag the version (or release candidate) in git, use the `make-tag.py` script from [briskcoin-maintainer-tools](https://github.com/briskcoin-core/briskcoin-maintainer-tools). From the root of the repository run:

    ../briskcoin-maintainer-tools/make-tag.py v(new version, e.g. 25.0)
...
Check out the source code in the following directory hierarchy.
    ...
    git clone https://github.com/briskcoin-core/guix.sigs.git
    git clone https://github.com/briskcoin-core/briskcoin-detached-sigs.git
    git clone https://github.com/briskcoin/briskcoin.git

### Write the release notes

Open a draft of the release notes for collaborative editing at https://github.com/briskcoin-core/briskcoin-devwiki/wiki.
...
Checkout the Briskcoin Core version you'd like to build:
...
Then open a Pull Request to the [guix.sigs repository](https://github.com/briskcoin-core/guix.sigs).
...
However if this is done, once the release has been tagged in the briskcoin-detached-sigs repo, the `guix-codesign` step must be performed again in order for the guix attestation to be valid when compared against the attestations of non-codesigner builds. The directories created by `guix-codesign` will need to be cleared prior to running `guix-codesign` again.

### Windows and macOS codesigners only: Commit the detached codesign payloads

```sh
pushd ./briskcoin-detached-sigs
...
- Detached signatures will then be committed to the [briskcoin-detached-sigs](https://github.com/briskcoin-core/briskcoin-detached-sigs) repository, which can be combined with the unsigned apps to create signed binaries.
...
- Upload to the briskcoincore.org server:
    1. The contents of each `./briskcoin/guix-build-${VERSION}/output/${HOST}/` directory.
...
- After uploading release candidate binaries, notify the briskcoin-core-dev mailing list and
  briskcoin-dev group that a release candidate is available for testing. Include a link to the release
  notes draft.
...
- Optionally help seed this torrent. To get the `magnet:` URI use:
  ...
  people without access to `briskcoincore.org` to download the binary distribution.
...
- Update the briskcoincore.org website

  - blog post

  - maintained versions [table](https://github.com/briskcoin-core/briskcoincore.org/commits/master/_includes/posts/maintenance-table.md)

  - RPC documentation update

      - See https://github.com/briskcoin-core/briskcoincore.org/blob/master/contrib/doc-gen/


- Update repositories

  - Delete post-EOL [release branches](https://github.com/briskcoin/briskcoin/branches/all) and create a tag `v${branch_name}-final`.

  - Delete ["Needs backport" labels](https://github.com/briskcoin/briskcoin/labels?q=backport) for non-existing branches.

  - Update packaging repo

      - Push the flatpak to flathub, e.g. https://github.com/flathub/org.briskcoincore.briskcoin-qt/pull/2

      - Push the snap, see https://github.com/briskcoin-core/packaging/blob/main/snap/local/build.md

  - Create a [new GitHub release](https://github.com/briskcoin/briskcoin/releases/new) with a link to the archived release notes

- Announce the release:

  - briskcoin-dev and briskcoin-core-dev mailing list

  - Briskcoin Core announcements list https://briskcoincore.org/en/list/announcements/join/

  - Briskcoin Core Twitter https://twitter.com/briskcoincoreorg
