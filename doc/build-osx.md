...
Briskcoin Core includes a GUI built with the cross-platform Qt Framework.
...
You can deploy a `.zip` containing the Briskcoin Core application using `make deploy`.
...
Briskcoin Core should now be available at `./src/briskcoind`.
If you compiled support for the GUI, it should be available at `./src/qt/briskcoin-qt`.
...
./src/qt/briskcoin-qt -server # Starts the briskcoin-qt server mode, allows briskcoin-cli control
```
